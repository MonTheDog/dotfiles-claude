...
Run activate.sh with the skin name, then show-banner.sh to display the banner:

```bash
~/.claude/skins/engine/activate.sh <name>
```

Then immediately show the banner by running:

```bash
~/.claude/skins/engine/show-banner.sh <name>
```

The output of show-banner.sh contains ANSI escape codes — run it via the Bash tool so the codes render in the user's terminal. Do NOT paraphrase or reformat the output — just run the command and let the raw output display.
